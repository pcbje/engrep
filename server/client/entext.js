...
mod.controller('MainCtrl', function($scope, $sce, $sanitize, $http) {
  $scope.response = {
  "results": [],
  "took": "0ms",
  "error": "",
...
$scope.clear = function() {
  ...
  var regex = new RegExp("<span class=\"yellow-[0-2]\">([^<]+)<\/span>", "g")
  $scope.text = $scope.text.replace(regex, "$1");
...
$scope.search = function() {
  ...
  $http.post('search?k=' + $scope.k, $scope.text).then(function(res) {
